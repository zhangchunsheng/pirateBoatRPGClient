...
(function(win){
   ...
   var _ABBox = win.ABBox = BBox.extend({
	   ...
	   collided:function(tBox)
	   {
         if(this.nx>tBox.mx||this.mx<tBox.nx)return false;
         if(this.ny>tBox.my||this.my<tBox.ny)return false;
		 return true;
	   },
	   show:function(ctx)
	   {
		 ...
		 ctx.lineWidth = 2;
	     ctx.strokeStyle = this.c;
    	 ctx.strokeRect(this.nx,this.ny,this.mx-this.nx,this.my-this.ny);
	   }
   }); 
   //凸多边形包围盒
