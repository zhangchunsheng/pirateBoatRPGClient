(function(BTG) {
	...
	BTG.CharacterAction.prototype.getRunKeyForTimeKey = function (timeKey, nKeyId) {
		if (this.m_acionData[nKeyId].length == 0) return null;
	...
	BTG.CharacterAction.prototype.parseXmlData = function (xmlDoc) {
		...
		cc.log(xmlDoc);
		for (var i = 0; i < BTG.CT_Count; i++) {
			var nodes = xmlDoc[BTG.s_ImageKeyName[i]];
			if (nodes === undefined)
				continue;
			var nodeCount = parseInt(nodes);
			if (nodeCount <= 0)
				continue;

			this.m_acionData[i].length = nodeCount;

			for (var iNode = 0; iNode < nodeCount; iNode++) {
				this.m_acionData[i][iNode] = new ActionData();

				var nodeData = xmlDoc[BTG.s_ImageKeyName[i] + iNode.toString()];
				if (nodeData === undefined) {
					var a;
				}
				this.m_acionData[i][iNode].m_time = parseFloat(nodeData["time"]) / 30.0;
				if (this.m_maxRunTime < this.m_acionData[i][iNode].m_time)
					this.m_maxRunTime = this.m_acionData[i][iNode].m_time;

				this.m_acionData[i][iNode].m_rot = parseFloat(nodeData["rot"]);
				this.m_acionData[i][iNode].m_pos.x = parseFloat(nodeData["posx"]);
				this.m_acionData[i][iNode].m_pos.y = parseFloat(nodeData["posy"]);
			}
		}
	}
})(BTG)